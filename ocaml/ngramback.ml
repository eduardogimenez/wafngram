...
(*
let rankDistance fld fldmdl1 fldmdl2 =
  let r1 = try Hashtbl.find fldmdl1 fld with Not_found -> 
 *)
...
let outlierFrequency dlen2 r1 r2 = outlier r1 r2
					   (*
...
let fieldDistance mdl test confld fld =
  try
	...
	(fun ngrm r2 l ->
	 ...
	 in  if (outlierFrequency dlen2 r1 r2) then
	       begin
		 (*assert (not ((meanDistance r1 r2) = 0.0 && Distrib.stdDev r1 = 0.0));*)
		 (BadFrequency ((*stringOfCategory*) ngrm,(meanDistance r1 r2),r1.mean,Distrib.stdDev r1))::l
	       end
	     else l)
	fldmdl2 [] 
...
let distance conf mdl test =
  List.sort
    (fun a b -> if a>b then 1 else if a=b then 0 else -1)
    (Hashtbl.fold
       (fun fld fldatt l ->
	let confld = Config.get conf fld in
	if confld.tokenization = Config.Exclude then l else (fld,fieldDistance mdl test confld fld)::l) test [])
...
(*
let testFileDistances conf mdl testfilemdl =
  List.map (fun testmdl -> distance conf mdl (buildModel conf true testmdl)) testfilemdl
 *)		      
...
let printTestFileDistances conf abs mdl ltestinst out =
